...
app.get('/home/libraryItems/loans/:clientEmail', (req, res) => {
...
app.get('/home/libraryItems/reservations/:clientEmail', (req, res) => {
  const ClientID = req.params.clientEmail
  const q = `SELECT 
    li.ItemID,
    li.Title,
    ...
    LEFT JOIN publisher p on li.Publisher_ID = p.Publisher_ID
    LEFT JOIN editor e on li.Editor_ID = e.Editor_ID
    LEFT JOIN reservation re on li.ItemID = re.ItemID
    WHERE re.Member_Email = ?;
  `
  ;
  db.query(q, [ClientID], (err, data) => {
    if (err) {
